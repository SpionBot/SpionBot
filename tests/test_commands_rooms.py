    ...
    if unwrapped.__closure__:
        for cell in unwrapped.__closure__:
            if callable(cell.cell_contents):
                return inspect.unwrap(cell.cell_contents)
    return unwrapped
...
def test_create_room_db_error(monkeypatch, fake_db, fake_context, make_update):
    async def fake_create(*args, **kwargs):
        return False

    monkeypatch.setattr(fake_db, "create_room", fake_create)
    update = make_update(user_id=1, text="/create")
    asyncio.run(_orig(commands.create_room)(update, fake_context))
    assert update.message.replies
...
def test_join_room_prompt_and_usage(fake_context, make_update):
    from handlers.button import get_main_keyboard

    join_text = get_main_keyboard().keyboard[0][1].text
    update = make_update(user_id=1, text=join_text)
...
def test_join_room_from_digits(fake_db, fake_context, make_update):
    fake_db.rooms["1234"] = {"id": "1234", "creator_id": 1, "game_started": False}
    fake_db.room_players["1234"] = [1]
    update = make_update(user_id=2, text="1234")
    fake_context.args = []
    asyncio.run(_orig(commands.join_room)(update, fake_context))
    assert update.message.replies
...
def test_join_room_creator_notify_fail(monkeypatch, fake_db, fake_context, make_update):
    fake_db.rooms["1234"] = {"id": "1234", "creator_id": 1, "game_started": False}
    fake_db.room_players["1234"] = [1]
    ...
    monkeypatch.setattr(fake_context.bot, "send_message", raise_send)
    update = make_update(user_id=2, text="/join 1234")
    fake_context.args = ["1234"]
    asyncio.run(_orig(commands.join_room)(update, fake_context))
...
def test_start_game_no_room(fake_context, make_update):
    ...
    asyncio.run(_orig_body(commands.start_game)(update, fake_context))
    assert update.message.replies
...
def test_start_game_not_enough_players(fake_db, fake_context, make_update):
    fake_db.rooms["1"] = {"id": "1", "creator_id": 1, "game_started": False, "mode": commands.DEFAULT_MODE, "word": None}
    fake_db.room_players["1"] = [1]
    update = make_update(user_id=1, text="/startgame")
...
def test_start_game_send_photo_errors(monkeypatch, fake_db, fake_context, make_update):
    fake_db.rooms["1"] = {"id": "1", "creator_id": 1, "game_started": False, "mode": commands.DEFAULT_MODE}
    fake_db.room_players["1"] = [1, 2]
    monkeypatch.setattr(
        commands,
    ...
    async def raise_photo(*args, **kwargs):
        raise RuntimeError("fail")

    monkeypatch.setattr(fake_context.bot, "send_photo", raise_photo)
    update = make_update(user_id=1, text="/startgame")
    asyncio.run(_orig(commands.start_game)(update, fake_context))
    assert fake_context.bot.sent_messages


def test_start_game_final_message_exception(monkeypatch, fake_db, fake_context, make_update):
    fake_db.rooms["1"] = {"id": "1", "creator_id": 1, "game_started": False, "mode": commands.DEFAULT_MODE}
    fake_db.room_players["1"] = [1, 2]
    monkeypatch.setattr(
        commands,
...
def test_restart_game_paths(fake_db, fake_context, make_update):
    ...
    asyncio.run(_orig_body(commands.restart_game)(update, fake_context))
    assert update.message.replies
...
def test_restart_game_notify_exception(monkeypatch, fake_db, fake_context, make_update):
    async def raise_send(*args, **kwargs):
        raise RuntimeError("fail")
...
def test_get_word_cached_and_exceptions(monkeypatch, fake_db, fake_context, make_update):
    fake_db.rooms["1"] = {"id": "1", "creator_id": 1, "game_started": True, "mode": commands.DEFAULT_MODE}
    fake_db.room_players["1"] = [1]
    spy_url = "https://i.pinimg.com/originals/41/15/70/4115707ee950d4b0aba69664f7986ae5.png"
    fake_db.image_cache[spy_url] = "spy_file"
    ...
    fake_db.player_data[(1, "1")] = {"role": "мирный", "word": "ALPHA", "card_url": "url"}
    fake_db.image_cache["url"] = "card_file"
    asyncio.run(_orig(commands.get_word)(update, fake_context))
    assert fake_context.bot.sent_photos

    fake_db.image_cache.pop("url", None)
    asyncio.run(_orig(commands.get_word)(update, fake_context))
    assert fake_context.bot.sent_photos
...
def test_show_players_success(fake_db, fake_context, make_update):
    fake_db.rooms["1"] = {"id": "1", "creator_id": 1, "game_started": False, "mode": commands.DEFAULT_MODE, "word": None}
    fake_db.room_players["1"] = [1]
    fake_db.player_data[(1, "1")] = {"role": "мирный"}
    update = make_update(user_id=1, text="/players")
    ...
    update = make_update(user_id=1, text="/players")
    asyncio.run(_orig_body(commands.show_players)(update, fake_context))
    assert update.message.replies


def test_leave_room_branches(monkeypatch, fake_db, fake_context, make_update):
    update = make_update(user_id=1, text="/leave")
    asyncio.run(_orig(commands.leave_room)(update, fake_context))
    assert update.message.replies
    ...
    update = make_update(user_id=1, text="/leave")
    asyncio.run(_orig(commands.leave_room)(update, fake_context))
    assert "1" not in fake_db.rooms

    fake_db.rooms["2"] = {"id": "2", "creator_id": 1, "game_started": False, "mode": commands.DEFAULT_MODE}
    fake_db.room_players["2"] = [1, 3]
    update = make_update(user_id=1, text="/leave")
    asyncio.run(_orig(commands.leave_room)(update, fake_context))
    assert fake_db.rooms["2"]["creator_id"] == 3

    async def raise_send(*args, **kwargs):
        raise RuntimeError("fail")

    monkeypatch.setattr(fake_context.bot, "send_message", raise_send)
    fake_db.rooms["3"] = {"id": "3", "creator_id": 1, "game_started": True, "mode": commands.DEFAULT_MODE}
    fake_db.room_players["3"] = [1, 4]
    update = make_update(user_id=1, text="/leave")
    asyncio.run(_orig(commands.leave_room)(update, fake_context))
...
def test_rules_show_cards_and_stats(monkeypatch, fake_db, fake_context, make_update):
    ...
    fake_db.rooms["1"] = {"id": "1", "creator_id": 1, "game_started": False, "mode": commands.MODE_DOTA}
    fake_db.room_players["1"] = [1]
    update = make_update(user_id=1, text="/rules")
    asyncio.run(_orig(commands.rules)(update, fake_context))
    assert update.message.replies
    ...
    monkeypatch.setattr(
        commands,
        "get_words_and_cards_by_mode",
        lambda mode: (list(range(25)), {str(i): "url" for i in range(25)}),
    )
    update = make_update(user_id=1, text="/cards")
    asyncio.run(_orig(commands.show_cards)(update, fake_context))
...
def test_set_mode_clash_dota(fake_db, fake_context, make_update):
    ...
    asyncio.run(_orig_body(commands.set_mode_clash)(update, fake_context))
    assert update.message.replies

    update = make_update(user_id=1, text="/mode")
    asyncio.run(_orig_body(commands.set_mode_dota)(update, fake_context))
    assert update.message.replies
